...
<head>
    ...
    <link href="https://fonts.googleapis.com/css?family=Raleway:100,100i,...etc" rel="stylesheet" />
    ...
    <div class="py-4 px-4 text-faded text-end">
        @if (auth()->check())
            <a href="{{ route('profile.index', auth()->user()->id) }}"><img
                    src="{{ file_exists(auth()->user()->profile_picture) ? asset(auth()->user()->profile_picture) : 'https://images.nightcafe.studio//assets/profile.png?tr=w-1600,c-at_max' }}"
                    class="rounded-circle shadow-4" style="width: 50px; height: 50px;" alt="Avatar" /></a>
            <span class="px-2" id="current_user">{{ auth()->user()->name }} {{ auth()->user()->surname }}
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav mx-auto">
                    <li class="nav-item px-lg-4"><a class="nav-link text-uppercase" href="{{ route('/') }}">Home</a>
                    </li>
